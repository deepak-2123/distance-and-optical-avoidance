...
SHOW_DISPLAY = False   # ← Set False on headless Pi (no monitor)
...
def main():
    ...
    print("=" * 50)
    print("  DRONE OBSTACLE AVOIDANCE — RUNNING")
    ...
    try:
        while True:
            ...
            frame = cam.get_frame()
            if frame is None:          # ← ADD null check
                continue
            ...
            if SHOW_DISPLAY:
                try:                   # ← Wrap in try — safe on headless
        ...
        cam.stop()
        shutdown()
        print(f"[Main] Frames processed: {loop_count}")
